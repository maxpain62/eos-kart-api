podTemplate(yaml: readTrusted('pod.yaml')) {
  node(POD_LABEL) {
    stage('Checkout Source') {
      ...
      git branch: 'main', url: 'https://github.com/maxpain62/eos-kart-api.git'
      script {
    ...
    stage ('build docker image') {
      container ('buildkit') {
        sh """
          ...
          --local dockerfile=.\
          --output type=image,name=134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/eos-kart-api:latest,push=true
          """
      }
    }
    stage ('package helm chart and push aws ecr repository') {
      container('aws-cli-helm') {
        sh """
          helm package eos-kart-api-chart && ls -l
          helm push eos-kart-api-0.1.0.tgz oci://134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/helm/
          aws ecr describe-images --repository-name dev/helm/eos-kart-api --region ap-south-1
          """
      }
    }
  }
